...
Europe_avg = unemployment.groupby('year')['Inflation'].mean().reset_index()

fig, ax = plt.subplots(figsize=(10.5, 6))

# parameters to apply_common_formatting
title = 'Inflation in Sweden and other european countries'
x_label = 'Year'
y_label = 'Inflation'
x_major = 2
x_minor = 1
...
# marking the period which was affected by Covid-19 the most
ax.axvspan(xmin=10, xmax=12, color='lightblue', alpha=0.3, label='Kryzys')
...
ax.plot(unemployment['year'].unique(), Sweden['Inflation'], label = 'Sweden')
ax.plot(Europe_avg['year'].unique(), Europe_avg['Inflation'], label = 'Europe average')
